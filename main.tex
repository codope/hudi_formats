...
// Need to motivate better. Lets take a look at abstract from https://vldb.org/pvldb/vol14/p3083-edara.pdf and refine our motivation.
I don't see much around why metadata has to be treated as first class citizen.
...
// lets draw parallels from very popular systems in the introduction para. We should add references to Snowflake, Redshift, BigQuery etc, on how they manage the metadata. and Hudi's is following bringing those to lakehouses.
// lets call out that indexing is very common technique in databases to optimize seeks for row access and to avoid scanning unnecessary data.
// Should we add some more hightlights of the metadata table design. like serverless, distributed, Consistent (ACID guaranteed). Snapshot isolation. Built for both batch and streaming ingestion. Infinitely scalable. Performant. Debuggability.
// Lets call out the scale to which metadata can grow? For eg, lets get details about one of the largest metadata table size in uber and quote that. This is to elucidate, that w/o metadata, serving data from the data table is close to unusable.
// Lets see if we can all out the append only nature/design(more like change logs) of metadata updates so as not to incur latencies to data table ingestion writers. I do see we have covered something on these lines in 2.1, but just wanted to call it out.
// Also, can we call out somewhere that, metadata is expected to reduce the amount of data scanned from data table, not necessarily give you the exact data files to be looked up. Based on the
// indexes being used, some additional data files could also be returned (corase grained indexes like col stats), but it is always guaranteed to ensure data consistency.
// Do we have space to call out "falsifiable expression" that we could see here https://vldb.org/pvldb/vol14/p3083-edara.pdf. We use similar technique in our col stats pruning.
...
// we need to revist this once we have nos from uber. Either it will be GCS or hdfs (RLI nos)
// Also for SI, let me collect the info from aditya. likely its EKS and not EMR.
...
The HFile-based layout with composite prefix keys reduces planning-time work from $O(\#\textit{table columns})$ to $O(\#\textit{query columns})$  (do we really need to stress on this again in the conclusion? not all indexes follow this pattern right? for eg, incase of RLI, its different.) for wide schemas, and concurrent indexing enables online backfill without pausing writers.
...
// For future work, can we also call out we plan to enhance how different indexes are used or chosen during query pruning. i.e. which index to use for a given query is not very intelligent as of now.
