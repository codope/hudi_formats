...
(2) \textbf{Execution} materializes index files and then reconciles commits that completed after the planned instant under a metadata-table lock, aborting on inconsistencies.
...
Future directions include bitmap indexes for low-cardinality columns, vector indexes for embedding similarity, adaptive index selection driven by observed workload patterns, and cross-format index interoperability so that indexes can be shared across engines and table formats.
